...
!  distance_cutoff = 1.432_REAL64 ! LAMMPS is working in LJ units not lattice units
  ...
  CALL GET_COMMAND_ARGUMENT(2, dump)

  READ(dump, *) distance_cutoff
  distance_cutoff = distance_cutoff*1_REAL64
  ...
  j = 1
  DO i = 1, N
     IF (conn_cluster_mat(i) .EQ. bg_clus) THEN
        cluster(j) = i
        j          = j + 1
     END IF
  END DO
  ...
  DO atom = 1, Nq6
     DO i = 1, Nq6
        ...
        IF (dist .LE. distance_cutoff) THEN
           ! dtp = DOT_PRODUCT(vec_ql_mat_rl(cluster(i), :), vec_ql_mat_rl(cls_tmp(atom), :))
           ! IF(dtp .GT. q6_threshold) THEN
           ! Connected - add to the cluster
